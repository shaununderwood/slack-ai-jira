...
class SlackService {
  ...
  constructor(config: SlackConfig, jiraService: JiraService) {
  ...
  async setupHandlers() {
    this.slackApp.event("app_mention", async ({ event, say }: { event: any, say: any }) => {
      if ("text" in event) {
    ...
    this.slackApp.event("message", async ({ event, say }: { event: any, say: any }) => {
      ...
      try {
        ...
        switch (parsed.intent) {
          case "create_ticket":
            await this.jiraService.createTicket({ summary: parsed.summary, projectKey: 'SCRUM' });
